...
Подключаемся к серверу с нового порта: `ssh имя_пользователя@ip_сервера -p номер_порта`. 
...
Можно также изменить URI путь в подписке json, чтобы предупреждение не маячило перед носом.
...
Для примера я сделал второе подключение, впишем идентичное название подписки в его клиенте.
...
Поскольку сертификат наш самоподписанный, мы проигнорируем ошибки, связанные с его проверкой. Настройки -> Основные настройки -> Подписка -> Ставим галочку на "Игнорировать ошибки TLS при обновлении подписок".
...
Переходим в 3x-ui, выбираем подключение и клиента, жмём на значок qr, затем на qr-код ПОДПИСКИ. Возвращаемся в Nekobox (Nekoray) -> Ctrl + V -> OK. В журнале нам говорят, что два профиля были импортированы. Можно подключиться к каждому из них.
