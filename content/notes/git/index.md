...
gir reset --mixed <commit_hash>         # Отмена коммита и убираем изменения из staged, не редактируя файлы
...
* **Привязать удалённый репозиторий** командой `git remote add origin <ссылка_на_удалённый_репозиторий>` и убедиться, что репозитории связаны: `git remote -v`  
...
3. Протестировать;
4. Перейти в главную ветку и слить ветку с главной: `git checkout main && git merge dev`. При этом обе ветки теперь будут указывать на последний коммит ветки `dev`;
